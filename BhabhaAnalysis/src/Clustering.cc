...
int Cluster::InsertHit(TMCVHit* hit, int ihit){
  if(nhits>MAXHIT-1){
...
void ClusterStructure::Clusterise(){
  for (int ii=0; ii <HitVec.size(); ++ii ) {
    ...
    for (int jj = 0; jj <ClusVec.size(); ++jj ) {
      if(HitVec.at(ii)->GetTime()-ClusVec.at(jj)->GetAverageTime()<4&&(HitVec.at(ii)->GetChannelId()-ClusVec.at(jj)->GetMostUpstreamChannel()==1||HitVec.at(ii)->GetChannelId()-ClusVec.at(jj)->GetMostDownstreamChannel()==-1)){	//4->parameter
...
void ClusterStructure::MergeClusters(){
  ...
  while(noCompact==0) {
    ...
    noCompact = 1;
    for(int ii = 0; ii < ClusVec.size(); ++ii){
      ...
      for(auto jj = ii+1; jj < ClusVec.size(); ++jj){
	...
	if(std::abs(iAvgT-jAvgT)<4&&(iUp-jDown==1||jUp-iDown==1)){
	  ...
	  ClusVec.erase(ClusVec.begin()+jj);//cluster jj has already been used so get rid of it, and move all other clusters down in the vector
	  noCompact = 0;
...
void ClusterStructure::HitSort(){
  ...
  std::vector<TMCVHit*> HitVecCopy(HitVec);
